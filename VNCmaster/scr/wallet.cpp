...
// venuscoin: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-venuscoin-address
...
// venuscoin: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CvenuscoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CvenuscoinAddress(address).ToString());
}
...
// venuscoin: check 'spent' consistency between wallet and txindex
// venuscoin: fix wallet spent state according to txindex
...
// venuscoin: disable transaction (only for coinstake)
