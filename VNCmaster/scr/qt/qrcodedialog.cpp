...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non VNC unit input in lnReqAmount, we generate the URI with VNC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(venuscoinUnits::format(venuscoinUnits::VNC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
